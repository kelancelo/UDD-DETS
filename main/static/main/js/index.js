...
async function handleRoundSelectChange(select) {
...
async function displayParticipants(roundId) {
    ...
    if (response.ok) {
        let participants = JSON.parse(await response.json());
...
async function handleParticipantClick(participant, roundId) {
    participantPanel.innerHTML = "";
...
async function getExtraParticipantData(div, roundId, participantId) {
